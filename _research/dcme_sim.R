...
config <- config::get(config = args[1], 
                      file = "_research/dcme_config.yml")
...
simulate <- function(n, config) {
    ...
    dcme(data, W, A, L, Z, M, Y, "binomial", folds = folds,
         control = .dcme_control(g_learners = config$g,
                                 p_learners = config$p,
                                 gamma_learners = config$gamma,
                                 mu_learners = config$mu,
                                 phi_learners = config$phi))
}

saveRDS(simulate(args[2], config$learners), 
        glue::glue("_research/data/dcme_{config$label}_{args[2]}_{id}.rds"))
